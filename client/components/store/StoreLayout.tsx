...
export type StoreVariant = { color: string; size: string; stock: number; active: boolean };
export type StoreProduct = Omit<typeof products[number], "nameEn"> & { nameEn?: string; description?: string; descriptionEn?: string; stock?: number; lowStockThreshold?: number; originalPrice?: number; salePrice?: number; badge?: string; colors?: string[]; sizes?: string[]; video?: string; images?: string[]; variants?: StoreVariant[] };
export type CartItem = { product: StoreProduct; quantity: number; size?: string; color?: string };
export const defaultStoreCategories = ["Sets", "Skirts / pants", "Blouses / shirts", "Denims", "Dresses"];
export type SiteSettings = { announcement: string; accent: string; activeTemplate?: "classic" | "new-design"; categories?: string[]; heroTitle?: string; heroDescription?: string; shippingAmount?: number; freeShippingThreshold?: number; shippingMessageAr?: string; shippingMessageEn?: string; returnsMessageAr?: string; returnsMessageEn?: string; madeInMessageAr?: string; madeInMessageEn?: string; walletNumber?: string; instapayNumber?: string; salesWhatsappNumber?: string; salesWhatsappUrl?: string; discoverVideos?: string[]; socialLinks?: { instagram?: string; facebook?: string; youtube?: string; whatsapp?: string; tiktok?: string } };
...
export type StoreOrder = { id: string; dbId?: string; date: string; total: number; subtotal?: number; discountAmount?: number; shippingAmount?: number; couponCode?: string; status: "جديد" | "قيد التجهيز" | "مكتمل" | "مرفوض"; items: number; customerName?: string; phone?: string; address?: string; notes?: string; paymentMethod?: PaymentMethod; transferNumber?: string; receipt?: string; orderItems?: StoreOrderItem[] };
...
export function StoreLayout({ children }: { children: ReactNode }) {
  ...
  const [siteSettings, setSiteSettings] = useState<SiteSettings>({ announcement: "", accent: "#d4775c", activeTemplate: "classic", categories: defaultStoreCategories, heroTitle: "New for Summer 2026", heroDescription: "Modest styles designed for everyday comfort.", shippingAmount: 80, freeShippingThreshold: 2500, shippingMessageAr: "شحن سريع داخل مصر", shippingMessageEn: "Fast shipping across Egypt", returnsMessageAr: "", returnsMessageEn: "", madeInMessageAr: "", madeInMessageEn: "", salesWhatsappNumber: "201068568250", salesWhatsappUrl: "https://wa.me/201068568250", socialLinks: { instagram: "", facebook: "", youtube: "", whatsapp: "https://wa.me/201553003040", tiktok: "" } });
  ...
  const storeCategories = siteSettings.categories?.length ? siteSettings.categories : defaultStoreCategories;
    ...
    fetch("/api/admin/orders", { credentials: "include" })
      ...
      .then((data: { orders?: Array<Record<string, unknown>> } | null) => {
        if (!cancelled && data?.orders) setOrders(data.orders.map((order) => ({
          id: String(order.order_number || order.id), dbId: String(order.id), date: String(order.created_at), total: Number(order.total), subtotal: Number(order.subtotal), discountAmount: Number(order.discount_amount), shippingAmount: Number(order.shipping_amount), couponCode: order.coupon_code ? String(order.coupon_code) : undefined, status: order.status === "completed" ? "مكتمل" : order.status === "processing" ? "قيد التجهيز" : order.status === "rejected" ? "مرفوض" : "جديد", items: Array.isArray(order.order_items) ? order.order_items.length : 0, orderItems: Array.isArray(order.order_items) ? order.order_items.map((item) => ({ name: String((item as Record<string, unknown>).name || ""), quantity: Number((item as Record<string, unknown>).quantity || 0), unitPrice: Number((item as Record<string, unknown>).unit_price || 0), total: Number((item as Record<string, unknown>).total || 0), size: String((item as Record<string, unknown>).size || "") || undefined, color: String((item as Record<string, unknown>).color || "") || undefined })) : [], customerName: String(order.customer_name || ""), phone: String(order.phone || ""), address: String(order.address || ""), notes: order.notes ? String(order.notes) : undefined, paymentMethod: order.payment_method as PaymentMethod,
        })));
      })
      .catch(() => undefined);
    return () => { cancelled = true; };
  ...
  const updateOrder = async (id: string, update: { status: "new" | "processing" | "completed" | "rejected"; paymentStatus?: "pending" | "verified" | "rejected"; internalNotes?: string }) => {
    const response = await fetch(`/api/admin/orders/${encodeURIComponent(id)}`, { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(update) });
    if (!response.ok) throw new Error("Unable to update order");
    const data = await response.json() as { order: Record<string, unknown> };
    const updated = data.order;
    setOrders((current) => current.map((order) => order.id === id ? { ...order, status: updated.status === "completed" ? "مكتمل" : updated.status === "processing" ? "قيد التجهيز" : updated.status === "rejected" ? "مرفوض" : "جديد" } : order));
  };
  ...
  return <StoreContext.Provider value={{ cart, cartItems, catalog, catalogStatus, catalogError, siteSettings, sections, pageSettings, coupons, appliedCouponCode, setAppliedCouponCode, orders, addToCart, removeFromCart, updateQuantity, clearCart, addProduct, updateProduct, deleteProduct, addOrder, updateOrder, updateSiteSettings, updateSection, updatePageSettings, addCoupon, deleteCoupon, liked, toggleLike, language, toggleLanguage }}>
    <main dir={isEnglish ? "ltr" : "rtl"} className="min-h-screen overflow-x-clip bg-white text-[#171717]">
      <div className="fixed inset-x-0 top-0 z-40 h-[30px] overflow-hidden border-b border-[#1c2822]/10 bg-[#f4f2e9] px-5 py-1.5 text-center text-[8px] tracking-[0.08em] text-[#1c2822]/75 sm:text-[9px]"><div className="announcement-track flex w-max items-center gap-16 whitespace-nowrap"><span>{siteSettings.announcement || (isEnglish ? `Free Shipping on Orders Over ${siteSettings.freeShippingThreshold ?? ""} EGP` : `شحن مجاني للطلبات فوق ${siteSettings.freeShippingThreshold ?? ""} جنيه`)}</span><span>{isEnglish ? "Enjoy Up to 50% Off · Welcome Anew" : "خصم يصل إلى ٥٠٪ · أهلاً بكِ"}</span><span>{siteSettings.announcement || (isEnglish ? `Free Shipping on Orders Over ${siteSettings.freeShippingThreshold ?? ""} EGP` : `شحن مجاني للطلبات فوق ${siteSettings.freeShippingThreshold ?? ""} جنيه`)}</span><span>{siteSettings.announcement || (isEnglish ? `Free Shipping on Orders Over ${siteSettings.freeShippingThreshold ?? ""} EGP` : `شحن مجاني للطلبات فوق ${siteSettings.freeShippingThreshold ?? ""} جنيه`)}</span><span>{isEnglish ? "Enjoy Up to 50% Off · Welcome Anew" : "خصم يصل إلى ٥٠٪ · أهلاً بكِ"}</span><span>{siteSettings.announcement || (isEnglish ? `Free Shipping on Orders Over ${siteSettings.freeShippingThreshold ?? ""} EGP` : `شحن مجاني للطلبات فوق ${siteSettings.freeShippingThreshold ?? ""} جنيه`)}</span></div></div>
      <header className={`z-30 ${isHome ? (isScrolled ? "fixed inset-x-0 top-[30px] border-b border-black/10 bg-[#eeece1] text-[#171717] shadow-sm" : "fixed inset-x-0 top-[30px] bg-transparent text-white") : "sticky top-[30px] border-b border-black/10 bg-[#eeece1] text-[#171717]"}`}>
        <div className="mx-auto flex h-[68px] max-w-[1240px] items-center justify-between px-5 lg:px-8">
          ...
          <nav className="hidden items-center gap-4 text-[10px] font-medium tracking-[0.08em] lg:flex">
            ...
            {storeCategories.map((category) => <Link key={category} to={`/shop?category=${encodeURIComponent(category)}`} className={`text-[13px] hover:underline hover:underline-offset-8 ${activeSection(`?category=${encodeURIComponent(category)}`) ? "font-semibold underline underline-offset-8" : ""}`}>{category}</Link>)}
          </nav>
