...
export type SiteSettings = { announcement: string; accent: string; heroTitle?: string; heroDescription?: string; productSizes?: string[]; discoverVideos?: string[]; socialLinks?: { instagram?: string; facebook?: string; youtube?: string; whatsapp?: string; tiktok?: string } };
...
export function StoreLayout({ children }: { children: ReactNode }) {
  ...
  const [siteSettings, setSiteSettings] = useState<SiteSettings>(() => { const defaults: SiteSettings = { announcement: "Free shipping on orders over 2,500 EGP · Cash on delivery available", accent: "#d4775c", heroTitle: "New for Summer 2026", heroDescription: "Modest styles designed for everyday comfort.", productSizes: ["S", "M", "L", "XL", "XXL"], socialLinks: { instagram: "", facebook: "", youtube: "", whatsapp: "https://wa.me/201553003040", tiktok: "" } }; try { const saved = JSON.parse(localStorage.getItem("no-name-settings") || "null") as SiteSettings | null; return { ...defaults, ...saved, socialLinks: { ...defaults.socialLinks, ...saved?.socialLinks } }; } catch { return defaults; } });
    ...
    <main dir={isEnglish ? "ltr" : "rtl"} className="min-h-screen overflow-hidden bg-white text-[#171717]">
      ...
      <header className={`z-30 ${isHome ? (isScrolled ? "fixed inset-x-0 top-[30px] border-b border-black/10 bg-[#eeece1] text-[#171717] shadow-sm" : "fixed inset-x-0 top-[30px] bg-transparent text-white") : "sticky top-[30px] border-b border-black/10 bg-[#eeece1] text-[#171717]"}`}>
        <div className="mx-auto flex h-[68px] max-w-[1240px] items-center justify-between px-5 lg:px-8">
          ...
          <Link to="/" className="absolute left-1/2 -translate-x-1/2 text-center leading-none" aria-label="الصفحة الرئيسية">
