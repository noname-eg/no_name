    ...
    fetch(endpoint, { credentials: "include" })
      ...
      .then((data: { order?: Record<string, unknown>; orders?: Record<string, unknown>[] } | null) => {
        ...
        setRemoteOrder({
          ...
          status: value.status === "completed" ? "مكتمل" : value.status === "processing" ? "قيد التجهيز" : value.status === "rejected" ? "مرفوض" : "جديد", items: Array.isArray(value.order_items) ? value.order_items.reduce((count, item) => count + Number((item as Record<string, unknown>).quantity || 0), 0) : 0, customerName: String(value.customer_name || ""), phone: String(value.phone || ""), address: String(value.address || ""), notes: value.notes ? String(value.notes) : undefined, paymentMethod: value.payment_method as StoreOrder["paymentMethod"], transferNumber: value.transfer_number ? String(value.transfer_number) : undefined, orderItems: Array.isArray(value.order_items) ? value.order_items.map((item) => { const row = item as Record<string, unknown>; return { name: String(row.name || ""), quantity: Number(row.quantity || 0), unitPrice: Number(row.unit_price || 0), total: Number(row.total || 0), size: row.size ? String(row.size) : undefined, color: row.color ? String(row.color) : undefined }; }) : [],
        });
      })
      .catch(() => undefined);
  }, [id]);
