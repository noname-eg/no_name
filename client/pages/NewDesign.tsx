...
function NewProductCard({ product, index }: { product: StoreProduct; index: number }) {
  ...
  return <article className="group">
    <div className="relative overflow-hidden rounded-[1.4rem] bg-[#d4cfc3]">
      <Link to={`/new-design/product/${product.id}`} className="block aspect-[.8]"><img src={product.image} alt={name} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></Link>
    ...
    <div className="mt-4 flex items-start justify-between gap-3"><div><Link to={`/new-design/product/${product.id}`} className="text-[12px] font-semibold hover:underline">{name}</Link><p className="mt-1 text-[11px] text-[#1c2822]/60">{product.category}</p></div><p className="text-[11px] font-medium">{getProductPrice(product, language)}</p></div>
  </article>;
}
...
export function NewDesignHome() {
  ...
  return <NewDesignLayout>
    ...
    <section className="mx-auto max-w-[1320px] px-5 pb-20 lg:px-10"><div className="mb-8 flex items-end justify-between"><div><p className="mb-2 text-[10px] uppercase tracking-[0.18em] text-[#1c2822]/50">{isEnglish ? "Selected pieces" : "قطع مختارة"}</p><h2 className="font-serif text-4xl tracking-[-.04em]">{isEnglish ? "The current mood" : "مزاج الموسم"}</h2></div><Link to="/new-design/shop" className="text-[11px] underline underline-offset-4">{isEnglish ? "View all" : "شاهدي الكل"}</Link></div><div className="grid grid-cols-2 gap-x-3 gap-y-10 sm:grid-cols-4 sm:gap-6">{featured.map((product) => <NewProductCard key={product.id} product={product} index={catalog.indexOf(product)} />)}</div></section>
  </NewDesignLayout>;
}
