...
export default function Admin() {
  ...
  const startNewProduct = () => { setForm(emptyProduct); setSizesInput((emptyProduct.sizes || []).join(", ")); setEditing(false); setActiveTab("products"); window.scrollTo(0, 0); };
  const editProduct = (product: StoreProduct) => { setForm({ ...product, colors: product.colors?.length ? product.colors : ["#222222"], sizes: product.sizes || [] }); setSizesInput((product.sizes || []).join(", ")); setEditing(true); setActiveTab("products"); window.scrollTo(0, 0); };
