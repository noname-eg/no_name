...
import { BarChart3, ImagePlus, LayoutDashboard, LogOut, Palette, Plus, Save, Settings, ShoppingBag, Trash2, Upload, X } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { defaultStoreCategories, useStore, type PageSettings, type SectionSettings, type SiteSettings, type StoreOrder, type StoreProduct } from "@/components/store/StoreLayout";
...
function parseVariants(value: string): NonNullable<StoreProduct["variants"]> {
  const variants = JSON.parse(value) as unknown;
  if (!Array.isArray(variants)) throw new Error("Invalid variants JSON");
  const normalized = variants.map((variant) => ({ color: String(variant.color || "").trim(), size: String(variant.size || "").trim(), stock: Number(variant.stock), active: variant.active !== false }));
  const keys = normalized.map((variant) => `${variant.color}::${variant.size}`);
  if (normalized.some((variant) => !variant.color || !variant.size || !Number.isInteger(variant.stock) || variant.stock < 0) || new Set(keys).size !== keys.length) throw new Error("Invalid variants JSON");
  return normalized;
}

async function uploadVideo(file: File) {
  const allowedTypes = ["video/mp4", "video/webm", "video/quicktime"];
  if (!allowedTypes.includes(file.type) || file.size > 20 * 1024 * 1024) throw new Error("Unsupported video");
  ...
  const response = await fetch("/api/admin/product-videos", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contentType: file.type, data }) });
  if (!response.ok) throw new Error("Unable to upload video");
...
export default function Admin() {
  ...
  const { catalog, siteSettings, sections, pageSettings, coupons, orders, addProduct, updateProduct, deleteProduct, updateSiteSettings, updateSection, updatePageSettings, addCoupon, deleteCoupon, updateOrder } = useStore();
  ...
  const [variantsInput, setVariantsInput] = useState(JSON.stringify(emptyProduct.variants || [], null, 2));
  ...
  const [orderFilter, setOrderFilter] = useState<"all" | "new" | "processing" | "completed" | "rejected">("all");
  ...
  const sales = orders.filter((order) => order.status !== "مرفوض").reduce((total, order) => total + order.total, 0);
  ...
  const storeCategories = settings.categories?.length ? settings.categories : defaultStoreCategories;
  const addCategory = () => { const category = newCategory.trim(); if (!category || storeCategories.includes(category)) return; setSettings((current) => ({ ...current, categories: [...storeCategories, category] })); setNewCategory(""); };
  const removeCategory = (category: string) => setSettings((current) => ({ ...current, categories: storeCategories.filter((item) => item !== category) }));
  const startNewProduct = () => { setForm(emptyProduct); setSizesInput((emptyProduct.sizes || []).join(", ")); setImagesInput(emptyProduct.images || [emptyProduct.image]); setVariantsInput(JSON.stringify(emptyProduct.variants || [], null, 2)); setEditing(false); setActiveTab("products"); window.scrollTo(0, 0); };
  const editProduct = (product: StoreProduct) => { const images = product.images?.length ? [...product.images] : [product.image]; setForm({ ...product, image: images[0] || product.image, images, colors: product.colors?.length ? product.colors : ["#222222"], sizes: product.sizes || [] }); setSizesInput((product.sizes || []).join(", ")); setImagesInput(images); setVariantsInput(JSON.stringify(product.variants || [], null, 2)); setEditing(true); setActiveTab("products"); window.scrollTo(0, 0); };
  ...
  const submitProduct = async (event: React.FormEvent) => {
    ...
    let variants: NonNullable<StoreProduct["variants"]>;
    try { variants = parseVariants(variantsInput); } catch { window.alert(isEnglish ? "Variants must be valid JSON with unique color and size pairs." : "بيانات المتغيرات يجب أن تكون JSON صحيحة بدون تكرار اللون والمقاس."); return; }
    ...
    setVariantsInput(JSON.stringify(emptyProduct.variants || [], null, 2));
    setEditing(false);
    notify();
  };
  ...
  const openOrder = async (order: StoreOrder) => {
    if (!order.dbId) return;
    try {
      const response = await fetch(`/api/admin/orders/${encodeURIComponent(order.dbId)}`, { credentials: "include" });
      ...
      if (typeof data.order.receipt === "string" && data.order.receipt.startsWith("receipts/")) {
        const receiptResponse = await fetch(`/api/admin/orders/${encodeURIComponent(order.dbId)}/receipt-url`, { credentials: "include" });
        if (receiptResponse.ok) setSelectedReceiptUrl(String((await receiptResponse.json() as { url: string }).url));
      }
    } catch { window.alert(isEnglish ? "Unable to load order details." : "تعذر تحميل تفاصيل الطلب."); }
  };
  ...
  const tabs = [
    ...
    { id: "orders", label: isEnglish ? "Orders" : "الطلبات", icon: ShoppingBag },
    { id: "reports", label: isEnglish ? "Sales reports" : "تقارير المبيعات", icon: BarChart3 },
  ];
  const tabGroups = [
    { title: isEnglish ? "Control center" : "مركز التحكم", items: [tabs[0], tabs[1]] },
    { title: isEnglish ? "Content" : "المحتوى", items: [tabs[2], tabs[3]] },
    { title: isEnglish ? "Marketing" : "التسويق", items: [tabs[4], tabs[5], tabs[6]] },
  ];
  ...
  return (
    <section className="admin-shell min-h-[calc(100vh-110px)] px-5 py-10 lg:px-8 lg:py-14">
      <div className="mx-auto max-w-[1320px]">
        ...
        {activeTab === "orders" && <div className="space-y-6"><div className="admin-panel p-6"><div className="panel-header"><h2>{isEnglish ? "Order review" : "مراجعة الطلبات"}</h2><select value={orderFilter} onChange={(event) => setOrderFilter(event.target.value as typeof orderFilter)} className={`${inputClass} mt-0 max-w-[180px]`}><option value="all">{isEnglish ? "All orders" : "كل الطلبات"}</option><option value="new">{isEnglish ? "New" : "جديد"}</option><option value="processing">{isEnglish ? "Processing" : "قيد التجهيز"}</option><option value="completed">{isEnglish ? "Completed" : "مكتمل"}</option><option value="rejected">{isEnglish ? "Rejected" : "مرفوض"}</option></select></div>{selectedOrder && <div className="mb-5 rounded border border-black/10 bg-white p-4 text-[11px]"><div className="mb-3 flex items-center justify-between"><h3 className="font-bold">{isEnglish ? "Order details" : "تفاصيل الطلب"}</h3><button type="button" onClick={() => { setSelectedOrder(null); setSelectedReceiptUrl(""); }} className="text-black/50">×</button></div><p>{String(selectedOrder.order_number || selectedOrder.id)} · {String(selectedOrder.customer_name || "")} · {String(selectedOrder.phone || "")}</p><p className="mt-1 text-black/60">{String(selectedOrder.address || "")}</p><div className="mt-3 space-y-2">{Array.isArray(selectedOrder.order_items) && selectedOrder.order_items.map((item) => <div key={String((item as Record<string, unknown>).id)} className="flex justify-between border-b border-black/5 pb-2"><span>{String((item as Record<string, unknown>).name)} · {String((item as Record<string, unknown>).size || "-")} / {String((item as Record<string, unknown>).color || "-")}</span><span>{String((item as Record<string, unknown>).quantity)} × {String((item as Record<string, unknown>).unit_price)}</span></div>)}</div>{selectedReceiptUrl && <a href={selectedReceiptUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block underline">{isEnglish ? "Open payment receipt" : "فتح إيصال الدفع"}</a>}</div>}<div className="mt-5 space-y-3">{orders.filter((order) => orderFilter === "all" || (order.status === "جديد" && orderFilter === "new") || (order.status === "قيد التجهيز" && orderFilter === "processing") || (order.status === "مكتمل" && orderFilter === "completed") || (order.status === "مرفوض" && orderFilter === "rejected")).map((order) => <div key={order.id} onClick={() => void openOrder(order)} className="grid cursor-pointer gap-3 border-b border-black/10 pb-4 text-[11px] md:grid-cols-[1.2fr_1fr_auto_auto] md:items-center"><div><strong>{order.id}</strong><p className="mt-1 text-black/55">{order.customerName} · {order.phone}</p><p className="mt-1 text-black/45">{new Date(order.date).toLocaleString(isEnglish ? "en-US" : "ar-EG")}</p></div><div><p>{order.total.toLocaleString("en-US")} ج.م</p><p className="mt-1 text-black/50">{order.paymentMethod || "-"}</p></div><select value={order.status === "قيد التجهيز" ? "processing" : order.status === "مكتمل" ? "completed" : order.status === "مرفوض" ? "rejected" : "new"} onChange={(event) => void updateOrder(order.dbId || order.id, { status: event.target.value as "new" | "processing" | "completed" | "rejected" }).catch(() => window.alert(isEnglish ? "Unable to update order." : "تعذر تحديث الطلب."))} className={`${inputClass} mt-0`}><option value="new">{isEnglish ? "New" : "جديد"}</option><option value="processing">{isEnglish ? "Processing" : "قيد التجهيز"}</option><option value="completed">{isEnglish ? "Completed" : "مكتمل"}</option><option value="rejected">{isEnglish ? "Rejected" : "مرفوض"}</option></select><button type="button" onClick={() => window.alert([order.customerName, order.address, order.notes || "", ...(order.orderItems || []).map((item) => `${item.name} × ${item.quantity}`)].filter(Boolean).join("\\n"))} className="border border-black/15 px-3 py-2">{isEnglish ? "Review" : "مراجعة"}</button></div>)}{!orders.length && <p className="text-[12px] text-black/50">{isEnglish ? "No orders yet." : "لا توجد طلبات بعد."}</p>}</div></div></div>}
