...
export default function Admin() {
  ...
  const startNewProduct = () => { setForm(emptyProduct); setEditing(false); setActiveTab("products"); window.scrollTo(0, 0); };
  const editProduct = (product: StoreProduct) => { setForm({ ...product, colors: product.colors?.length ? product.colors : ["#222222"], sizes: product.sizes?.length ? product.sizes : ["S", "M", "L"] }); setEditing(true); setActiveTab("products"); window.scrollTo(0, 0); };
  ...
  const submitProduct = (event: React.FormEvent) => {
    ...
    const product: StoreProduct = {
      ...
      price: `${Number(form.numericPrice).toLocaleString("en-US")} ج.م`,
